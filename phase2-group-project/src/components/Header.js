...
function Header({currentUser}) {
  ...
  <div className="w-screen h-[120px] z-10 bg-slate-600 justify-center items-center">
      <div className="justify-center items-center">
        ...
        <div>{((currentUser === "") ? (<NavLink to="/login" exact><span className="text-white text-base pt-4 p-3"></span></NavLink>) : (<span className="text-white text-base pl-3">Welcome, {currentUser}</span>))}</div>

        <ul className="flex flex-initial h-[120px] z-10 bg-slate-600 items-center justify-center mx-auto">
